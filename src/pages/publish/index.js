...
class Publish extends Component {
  ...
  adoptionChangeHandler(code, name, active) {
    ...
    const active_arr = active.target.value;
    const _active = active_arr.filter(activeItem => activeItem["name"] === name["name"])[0]["active"];
    tagCodes.indexOf(code) === -1 ? tagCodes.push(code) : tagCodes.splice(tagCodes.indexOf(code), 1);
    setAttrValueHandler({
      isAdoption: !_active,
      tagCodes
    });
  ...
  render() {
    const {publishStore} = this.props;
    const {adoptionChangeHandler} = this;
    const {
      isRefusedModal,
      content,
      files,
