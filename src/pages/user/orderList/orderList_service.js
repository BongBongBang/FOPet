      ...
      Tools.requestPaymentConfig({
        ...
        success: (res) => {
          this.setState({
            ...
            if (orderTypeConfig[type]['isMedical']) {
              consult.call(this);
            } else {
  ...
  return Tools.request({
    ...
    success: (data, header) => {
      if (data) {
        //清除应用内所有的页面,然后跳转到结果页
        Taro.reLaunch({
          url: `${pageCurrentList[14]}?content=${consultConfig.content}&title=${consultConfig.title}&url=${consultConfig.url}&name=${consultConfig.name}`
        });
      }
    },
    fail: res => {
 ...
 * @returns {*}
 */
function completeOrderList(orderId = 0, order = {}) {
  const params = JSON.stringify(order) || '{}';
  return Tools.request({
    url: `eshop/order/complete/${orderId}`,
    success: (data) => {
      Taro.navigateTo({
        url: `${pageCurrentList[36]}?order=${encodeURIComponent(params.replace(/%/g, '%25'))}`
      });
    },
    fail: (res) => {
      this.initialLoading();
