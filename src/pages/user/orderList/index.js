...
class OrderList extends Component {
  ...
  onRedirectToPlaceOrderDetail = (e) => {
    ...
    Taro.navigateTo({
      url: orderTypeConfig[orderType]['url'](function (position) {
        return `${pageCurrentList[position]}?order=${encodeURIComponent(item)}&orderNo=${orderNo}&orderId=${id}&docId=${docId}&consultType=${consultType}&uploadConsultImages=${uploadConsultImages}&consultContent=${consultContent}&orderType=${orderType}`;
      })
    });
    //取消冒泡事件
    e.stopPropagation();
  ...
  evaluationHandler = (e = {}, item = {}) => {
    ...
    Taro.navigateTo({
      url: `${pageCurrentList[38]}?order=${params}`
    });
    //取消冒泡事件
    e.stopPropagation();
  };
