...
class PlaceOrder extends Component {
  ...
  componentWillMount() {
    ...
    const {fosterService = null, otherService = null, startDate = null, endDate = null} = params;
    const startTime = Number(startDate);
    const endTime = Number(endDate);
    const durationTime = Math.round(dayjs(endTime).diff(dayjs(startTime), 'day', true));
    this.setState({
  ...
  onDurationChange = () => {
    ...
    this.setState({
      durationTime: Math.round(dayjs(moveEndTime).diff(dayjs(moveStartTime), 'day', true))
    }, () => {
  ...
  onAccountChangeHandler = (type, duration) => {
    ...
    service.forEach(item => {
      let total = 0,
        orderItem = [];
      if (item.checked) {
        total += parseFloat((item.goodsNum * item.discPrice * durationTime).toFixed(2));
        if (item.goodsNum > 0) {
          orderItem = [...orderItem, {
            goodsId: item.goodsId,
            skuId: item.optionGroup[0] ? item.optionGroup[0]['skuId'] : 0,
  ...
  render() {
    const {statusBarClassName = '', isX = false} = Tools.adaptationNavBar();
    const {
      fosterService = [],
      otherService = [],
      contact = null,
    ...
    const startFormatDate = dayjs(startTime).format('YYYY-MM-DD'),
      endFormatDate = dayjs(endTime).format('YYYY-MM-DD'),
      moveStartFormatDate = dayjs(moveStartTime).format('YYYY-MM-DD'),
      moveEndFormatDate = dayjs(moveEndTime).format('YYYY-MM-DD'),
    ...
    return (
      <View className='pet-placeOrder'>
        ...
        <View className='pet-placeOrder-container'>
          <View className={cns(
            'pet-placeOrder-area',
            ...
            <View className='pet-placeOrder-area-service'>
              {
                fosterService && fosterService.length > 0 && fosterService.map((fosterItem, fosterIndex) => {
                  const optionGroups = fosterItem.optionGroup;
                  return (
            ...
            <View className='pet-placeOrder-area-service'>
              {
                otherService && otherService.length > 0 && otherService.map((otherItem, otherIndex) => {
                  const optionGroups = otherItem.optionGroup;
                  return (
                    <View className='pet-placeOrder-area-service-item'
          ...
          <View className={
            ...
            <View
              className={cns(
                'pet-placeOrder-area-agreement-wrap',
                {
                  'pet-placeOrder-area-agreement-wrapX': !!isX
                }
              )}
            >
              <AtCheckbox
          ...
          <View className={cns(
            'pet-placeOrder-account',
            {
              'pet-placeOrder-accountX': !!isX
            }
          )}>
            <AtButton className={cns(
              'pet-placeOrder-account-total',
              {
                'pet-placeOrder-account-totalX': !!isX
              }
            )}
                      full
                      type='primary'
            ...
            <AtButton className={cns(
              'pet-placeOrder-account-ordered',
              {
                'pet-placeOrder-account-orderedX': !!isX
              }
            )}
                      full
                      type='primary'
