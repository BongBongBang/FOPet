  ...
  return Tools.request({
    ...
    success: (data = {}) => {
      ...
      const startTimeDesc = dayjs(startTime).format('YYYY年MM月DD日'),
        ...
        durationMinutes = dayjs(paymentEndTime).diff(dayjs(nowTime), 'minutes'),
        durationSeconds = dayjs(paymentEndTime).diff(dayjs(nowTime), 'seconds') - (durationMinutes * 60),
        paymentTimeDesc = paymentTime ? dayjs(paymentTime).format('YYYY-MM-DD HH:mm:ss') : status.noPayment,
