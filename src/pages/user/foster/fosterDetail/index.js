...
class FosterDetail extends Component {
  ...
  onPlaceHolderHandler = async (event) => {
    ...
    if (!authSetting['scope.userInfo']) {
      Taro.navigateTo({
        url: `${pageCurrentList[21]}?pages=fosterDetail`
      });
    } else {
      Taro.navigateTo({
        url: `${pageCurrentList[27]}?goodsName=${goodsName}&fosterService=${JSON.stringify(fosterService)}&otherService=${JSON.stringify(otherService)}&startDate=${startDate * 1000}&endDate=${endDate * 1000}`
      });
    }
