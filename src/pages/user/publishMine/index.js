...
import {CardView, EmptyView} from '../../../components/bussiness-components';
...
class publishMine extends Component {
  ...
  getPetPublishMineDetailHandler = (id, avatarUrl, nickName) => {
    Taro.navigateTo({
      url: `${pageCurrentList[6]}?id=${id}&avatarUrl=${avatarUrl}&nickName=${nickName}&page=publishMine`
    });
  };
    ...
    return (
      <View className='pet'>
        ...
        }
        {
          ...
          /> : <EmptyView
            className='pet-publishMine-empty'
            icon='petPlanet-cat-ao'
            prefix='iconfont'
            color='#000'
            size={48}
            description='啊哦~铲屎官搜索不到喵星人~~'
          />
        }
      </View>
    )
