...
class Message extends Component {
  ...
  messageScrollListener = (event) => {
    ...
    Taro.createSelectorQuery().select('#pet-message').fields({
      ...
      if ((((scrollHeight - domHeight - scrollTop) / scrollHeight) <= 0.05) && (length < total) && !isNext) {
        Tools.run(function* () {
          this.isNext = true;
          yield this.setState({
            isShowLoad: true
        ...
        >
          ...
          }
          {
            consultationsList.length > 0 ? consultationsList.map((consultationItem, consultationIndex) => {
                  ...
                  <ListItemView
                    ...
                    renderExtra={
                      ...
                      >
                        ...
                        >
                          {
                            dayjs(consultationItem['createTime'], 'YYYY-MM-DDTHH:mm:ss.000ZZ').format('YYYY-MM-DD HH:mm')
                          }
                        </View>
                        <View>
                          <AtTag
              ...
              className='pet-message-empty'
              title='宠物问题，专业咨询'
              description='啊哦~您还没有任何宠物问题的咨询'
              prefix='iconfont'
              color='#000'
              size={48}
