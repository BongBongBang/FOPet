  ...
  return Tools.request({
    ...
    success: data => {
      const list = data.communications.data,
        ...
        imgList = list.filter(item => (item && item['msgType']) === constants.msgType.IMG).map(item => item.content);
      this.setState(() => {
        return {
          pageNum: ++pageNum,
    ...
    return Tools.request({
      ...
      success: data => {
        const list = data.data,
          ...
          imgList = _communicationsList.filter(item => (item && item['msgType']) === constants.msgType.IMG).map(item => item.content);
        this.setState({
          total,
