...
function receiveRedPacket() {
  ...
  return Tools.request({
    ...
    success: (data = {}, header = {}) => {
      ...
      if (success) {
        const time = dayjs(createTime, 'YYYY-MM-DDTHH:mm:ss.000ZZ').format('YYYY-MM-DD HH:mm:ss');
        Taro.navigateTo({
          url: `${pageCurrentList[31]}?redPackType=${redPackType}&totalAmount=${totalAmount}&avatar=${avatar}&nickName=${nickName}&time=${time}`
        });
      }
    },
    fail: (res) => {
