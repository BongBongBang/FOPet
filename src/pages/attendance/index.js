...
import {setAttrValue} from '../index/home_action';
...
import {staticData, pageCurrentList, loadingStatus, linkTypeHandler, petPlanetPrefix} from '../../constants';
...
class Attendance extends Component {
	...
	attendanceContainerScrollListener = async () => {
		let {
			attendanceStore: {pageNum, total, signList, throttle},
			setAttrValue,
			communityGroupLogsHandler
		} = this.props;
	...
	getUserInfoHandler = async (res = {}) => {
		...
		const {
			commonUserInfoHandler = () => {
			}
		} = this;
		const {detail = {}, isNeedUpdate = true} = res;
		...
		if (userInfo) {
			...
			setAttrValue({
				isPermission: !isNeedUpdate
			});
			this.setState({
				loading: false
			});
			userTinyHomeInfoHandler.call(this);
			commonUserInfoHandler();
		}
	};
	...
	onRedPacketChangeHandler = () => {
		...
		onModalChangeHandler('isShowCancel');
	}
	...
	render() {
		...
		const {
			attendanceStore: {
				...
				signObject,
				signObjectType,
				modal: {sign: {isSigned, data: {remarks}}}
			}, setAttrValue
		} = this.props;
		const {
			getUserInfoHandler = () => {
				...
				}
				{
					...
					>
						...
						<View className='pet-attendance-header'>
							<View className='pet-attendance-header-rules'
										onClick={() => onModalChangeHandler('isShowRule')}>
								<AtIcon
							...
							>
								...
								<View>
									<Image
										...
										mode='widthFix'
									/>
									金币 <Text className='pet-attendance-content-header-item'
													 decode>{coin ? coin : 0}&nbsp;枚</Text>
									<AtTag
										type='normal'
								...
								>
									{
										signed ? <Block>
											...
											<View className='at-col'>
												...
												<View
													className='pet-attendance-content-accordion-detail-row'>打卡时间: {signTime}</View>
											</View>
															...
															</View>
															{
																signItem[1].map((infoItem, infoIndex) => {
																	...
																	return (
																		...
																		>
																			<View
																				className='pet-attendance-list-component-container-content-row-aside'>
																				<Image
																			...
																			<View
																				className='pet-attendance-list-component-container-content-row-message'>
																				<Text
																				...
																				<View
																					className='pet-attendance-list-component-container-content-row-signTime'>
																					<Text
																						decode
																					>
																						打卡时间
																			...
																			</View>
																			{
																				infoItem['lucky'] &&
																				<View
																					className='pet-attendance-list-component-container-content-row-redPacket'>
																					<Image
																						src={imgs.redPacket}
																						mode='widthFix'
																					/>
							...
							>
								...
								<View onClick={(e) => e.stopPropagation()}>
									<Image
										...
										onClick={() => onModalChangeHandler('isShowCancel')}
									/>
								</View>
							</View>
						</ModalView>
						...
						<ModalView
							...
							onClose={() => onModalChangeHandler('isShowRule')}
						>
							...
							<View className='at-modal__footer'>
								<View className='at-modal__action'>
									<Button onClick={() => onModalChangeHandler('isShowRule')}>
						...
						>
							<AtFab
								...
								name='hypermarket'
								onClick={navigateToLdmMall}
							>
								<View
									className={cns(
										'pet-attendance-fab-inner',
										'pet-attendance-fab-inner-hypermarket-text'
									)}
								>
							...
							>
								...
								>
									<View
										className={cns(
											'pet-attendance-fab-inner',
											'pet-attendance-fab-inner-medicalAdvice-text'
										)}
									>
										<Image className='pet-attendance-fab-inner-icon'
