...
class BargainDetail extends Component {
	...
	onEnsureBuyClick = async (e = {}) => {
		...
		let {data: isNeedUpdate = true} = await bargainDetailAPI.getNeedUpdateUserInfo();
		//保存判断用户是否需要更新信息的状态
		this.setState({
			isNeedUpdate
		});
		...
		return (
			<View className='bargainDetail'>
				...
				>
					...
					>
						...
						>
							<View class='bargainDetail-sku-choose-info-price'>
								<Text
									class='bargainDetail-sku-choose-info-price-desc'>当前价:</Text> &#165;{parseFloat(salePrice / 100)}
							</View>
					...
					<View className={cns(
						'bargainDetail-sku-choose-btnArea',
						{
							'bargainDetail-sku-choose-btnAreaX': !!isX
						}
					)}>
						<View
							className='bargainDetail-sku-choose-btn'
