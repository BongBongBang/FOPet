...
import {tabBarTabList, pageCurrentList, cnsltState} from "../../utils/static";
...
@connect((state, ownProps) => {
  ...
  return {
     ...
     */
    changeCurrentInit() {
      dispatch(changeCurrent({current: 2}));
    },
    /**
     * 前往对话记录页面
     * @尹文楷
...
class Message extends Component {
  ...
  componentDidShow() {
    (async () => {
      ...
      //获取TabBar隐含消息tab信息,并保存在storage中
  ...
  render() {
    ...
    let {consultationsList, isLoading, tabBarInfo} = this.state;
    //筛选出dot为true的底部边栏结构
    filterDotTabBarList(tabBarInfo);
    return (
      <View
