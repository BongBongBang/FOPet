...
class LdmMall extends Component {
  ...
  onFilterMall = (current = 0) => {
    ...
    this.setState({
      current,
      searchType: searchTypeConfig[current]
    }, () => {
  ...
  onDirectToGoodsList = (e = {}) => {
    ...
    this.setState({
      searchType: constants['searchTypeConfig'][current]
    }, () => {
      Taro.navigateTo({
        url: `${pageCurrentList[39]}?id=${categoryId}&searchType=${constants['searchTypeConfig'][current]}&name=${categoryName}`
      });
    });
    //取消冒泡事件
    e.stopPropagation();
  ...
  onDirectToSearchGoodsList = (e) => {
    ...
    Taro.navigateTo({
      url: `${pageCurrentList[39]}?searchType=${searchType}&para=${para}`
    });
    //取消冒泡事件
    e.stopPropagation();
  };
  ...
  onFocusHandler = (event = {}) => {
    const {searchTypeConfig = {}} = constants;
    this.setState({
      searchType: searchTypeConfig[2]
    });
  };
