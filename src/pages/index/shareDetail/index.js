...
class ShareDetail extends Component {
  ...
  onShareAppMessage({from, target, webViewUrl}) {
    ...
    return {
      title: `逼疯的铲屎官-${title}`,
      path: `${pageCurrentList[7]}?id=${id}&page=${page}&nickName=${nickName}&avatarUrl=${avatarUrl}`,
      imageUrl: images[0]
    }
  }
  ...
  setCollectionConfig = async () => {
    ...
    if (currentPages[length - 2] && pageCurrentList[6].indexOf(currentPages[length - 2]['route']) !== -1) {
