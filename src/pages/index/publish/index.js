...
class Publish extends Component {
  ...
  onGetUserInfoHandler = async (event) => {
    ...
    const {verify} = this;
    const {data: isNeedUpdate = true} = await publishAPI.getNeedUpdateUserInfo();
    //保存判断用户是否需要更新信息的状态
    this.setState({
      isNeedUpdate
    });
  ...
  adoptionChangeHandler = (code, {name, active}) => {
    ...
    const filterIndex = tagCodes.indexOf(code);
    tagCodes.splice(filterIndex, 1);
    setAttrValueHandler({
      isAdoption: !active,
      tagCodes: !active ? [...tagCodes, code] : [...tagCodes]
    });
  };
