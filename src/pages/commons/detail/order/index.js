...
class CommonOrder extends Component {
	...
	componentWillMount() {
		...
		let _item = JSON.parse(decodeURIComponent(item)) || [];
		_item = _item.map(item => {
			const option = item.orderItem.optionGroup.find(groupItem => groupItem.skuAttributeId === constants.IDCardId);
			return {
				...item,
				goodsNum: item.orderItem.goodsNum,
				remark: '',
				isFullIDCard: !!option && !!option.optionName
			};
		});
		this.setState({
			item: _item
	...
	verify = () => {
		...
		const filterOption = item.filter(item => item['isFullIDCard']);
		return Tools.addRules([
			address,
			!!parseFloat(totalPrice),
			!(filterOption.length > 0 && !remark)
		], [{
			rule: 'isEmpty',
	...
	render() {
		...
		const isFullIDCard = item.findIndex((_item = {}) => {
			return _item['isFullIDCard'];
		}) !== -1;
				...
				<View className='order'>
									...
									>
										...
										<View className='order-detail-content'>
											...
											<View className='order-detail-content-desc'>
												{_item.orderItem.optionGroup.map(item => item.optionName).join(' ')}
											</View>
										</View>
					...
					<View className='order-remark'>
						...
						<View className='order-remark-content'>
							<AtInput
								placeholder={isFullIDCard ? constants.order.placeholder.IDCardRemark : constants.order.placeholder.remark}
								placeholderStyle='font-size:16PX;font-weight:400;color:#999;'
								className='order-remark-content-input'
								value={remark}
								onChange={(e) => onChangeHandler('remark', e)}
							/>
						</View>
					</View>
