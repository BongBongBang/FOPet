...
class CommonOrderDetail extends Component {
  ...
  componentWillMount() {
    const {params: {orderNo = '', orderId = 0}} = this.$router;
    this.setState({
      orderNo,
      orderId
    });
  ...
  render() {
    ...
    const orderInfo = constants.orderInfo;
    const paymentPoint = payment.indexOf('.'),
      paymentInt = payment.slice(0, paymentPoint),
    ...
    return (
      <View className='orderDetail'>
        ...
        </View>
        {
          items && items.length > 0 && items.map(_item => {
            const discPrice = parseFloat(_item.discPrice).toFixed(2),
              ...
              discIntPrice = discPrice.slice(0, discPricePoint),
        ...
        <View className={cns('orderDetail-service', 'orderDetail-info')}>
        ...
        <View className='orderDetail-serviceOrder'>
          <AtButton className={
            cns(
              ...
              {
                'orderDetail-serviceOrder-isFull': orderStatus !== status.pendingPayment
              }
            )
          }
                    full
