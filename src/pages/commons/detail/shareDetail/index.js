...
class ShareCommonDetail extends Component {
  ...
  onEnsureBuyClick = e => {
    ...
    if (verify()) {
      ...
      Taro.navigateTo({
        url: `${pageCurrentList[33]}?item=${encodeURIComponent(JSON.stringify(navParam))}`
      });
    }
    //取消冒泡事件
    e.stopPropagation();
  ...
  onCommentScrollHandler = ({detail: {scrollHeight = 0, scrollTop = 0}}) => {
    ...
    Taro.createSelectorQuery().select('#detail-comments').fields({
      ...
      if (((scrollHeight - (scrollTop + height)) / scrollHeight < 0.1) && (length < total) && isNext) {
        this.isNext = false;
        this.setState({
          isShowLoad: true,
          loadStatus: staticData.loadStatusConfig.loading
  ...
  renderDetail = () => {
    ...
    return (
      <ScrollView
        ...
        scrollY
        style={{height: `calc(100% - ${88 + (refs ? refs.height : 0)}px)`}}
      >
        ...
        <View className='detail-goods'>
          ...
          <View className={
            cns(
              ...
              'detail-goods-goodsDetail-share'
            )
          }>
    ...
    >
      {
        ...
        }) : <View className='detail-commentsArea-empty'>
          <AtIcon
            className='detail-commentsArea-icon'
            prefixClass='iconfont'
            value='petPlanet-cat-ao'
            color='#000'
            size={48}
          />
          <View className='detail-commentsArea-empty-title'>
            啊哦~~~
          </View>
          <View className='detail-commentsArea-empty-description'>
            铲屎官没有发现任何评论
          </View>
        </View>
      }
