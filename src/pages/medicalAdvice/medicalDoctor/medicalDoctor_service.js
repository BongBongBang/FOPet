...
import {consultConfig, pageCurrentList} from '../../../constants';
  ...
  return Tools.request({
    ...
    success: (data, header) => {
      if (data) {
        //清除应用内所有的页面,然后跳转到结果页
        Taro.reLaunch({
          url: `${pageCurrentList[15]}?content=${consultConfig.content}&title=${consultConfig.title}&url=${consultConfig.url}&name=${consultConfig.name}`
        });
      }
    },
    fail: res => {
  ...
  return Tools.request({
    ...
    success: (data) => {
      ...
      Tools.requestPaymentConfig({
        ...
        fail: (res) => {
          Taro.redirectTo({
            url: `${pageCurrentList[30]}?docId=${docId}&consultType=${consultType}&consultContent=${consultContent}&uploadConsultImages=${JSON.stringify(uploadConsultImages)}`
          });
        },
        complete: (res) => {
          this.initialLoading();
