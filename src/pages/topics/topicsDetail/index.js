...
class TopicsDetail extends Component {
	...
	onFocusHandler = async (value) => {
		...
		const {data: isNeedUpdate = true} = await topicsAPI.getNeedUpdateUserInfo();
		//保存判断用户是否需要更新信息的状态
		this.setState({
			isNeedUpdate,
			isFocus: true
	...
	render() {
		...
		const {navBarHeight = 0, statusBarClassName = '', isX = false} = Tools.adaptationNavBar();
		...
		return (
			...
			>
										...
										>
											...
											<View className='pet-topic-detail-comments-content-item-view'>
												...
												<View className='pet-topic-detail-comments-content-item-view-time'>
													{dayjs(commentItem['createTime'], 'YYYY-MM-DDTHH:mm:ss.000ZZ').format('YYYY-MM-DD HH:mm')}
												</View>
												...
												</Text>
												{
													...
													<View
														className='pet-topic-detail-comments-content-item-view-subComments'>
														{
															subComments.map(item => {
																return (
																	...
																	>
																		...
																		/>
																		<View
																			className='pet-topic-detail-subComments-content'>
																			<View
																				className='pet-topic-detail-subComments-content-title'>
																				<Text>{item['nickNameFrom']}</Text> 回复 <Text>{commentItem['nickNameFrom']}</Text>
																			</View>
																			<Text
																				className='pet-topic-detail-subComments-content-info'
																				selectable
																			>
																				{item['content']}
																			</Text>
														...
														{
															commentItem['moreSubComments'] &&
															<View className='pet-topic-detail-subComments-more'
				...
				<View className={cns('at-row',
					...
					{
						'pet-topic-detail-topicBarX': !!isX
					}
				)}
